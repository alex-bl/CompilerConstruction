...
void mCc_print_dot_statement_if(struct mCc_ast_statement *statement, void *data)
{
	...
	mCc_ast_print_dot_edge_if_dest_exists(out, statement,
	                                      statement->if_statement, "if true");
	mCc_ast_print_dot_edge_if_dest_exists(
	    out, statement, statement->else_statement, "otherwise");

	mCc_ast_print_dot_edge_if_dest_exists(out, statement,
	                                      statement->next_statement, "next");
}
                                   ...
                                   void *data)
{
	...
	mCc_ast_print_dot_edge(out, statement, statement->loop_condition_expression,
	                       "condition");
	mCc_ast_print_dot_edge_if_dest_exists(
	    out, statement, statement->while_statement, "statement");
